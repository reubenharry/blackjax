...
class PersistentSMCState(NamedTuple):
    """Current state for the persistent SMC algorithm.
...
def init(particles: ArrayLikeTree):
    ...
    return PersistentSMCState(particles, weights, 0.0)
...
def build_kernel(
    ...
    """Build the base Persistent SMC kernel.

    Persistent SMC uses tempering to sample from a distribution given by
    ...
    Returns
    -------
    A callable that takes a rng_key and a PersistentSMCState that contains the current state
    ...
    def kernel(
        ...
        """Move the particles one step using the Persistent SMC algorithm.
        ...
        state
            Current state of the persistent SMC algorithm
        lmbda
            Current value of the tempering parameter
        mcmc_parameters
        ...
        -------
        state
            The new state of the persistent SMC algorithm
        info
        ...
        def persistent_logposterior_fn(position: ArrayLikeTree) -> float:
            logprior = logprior_fn(position)
            persistent_loglikelihood = state.lmbda * loglikelihood_fn(position)
            return logprior + persistent_loglikelihood
        ...
        smc_state, info = update_particles(
            ...
            num_mcmc_steps,
            mcmc_parameters,
            persistent_logposterior_fn,
            log_weights_fn,
        )

        persistent_state = PersistentSMCState(
            smc_state.particles, smc_state.weights, state.lmbda + delta
        )

        return persistent_state, info
